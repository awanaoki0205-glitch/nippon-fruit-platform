...
class NF_Intelligence_Dashboard {
    ...
    private static function analyse() {
        ...
        $a=array(
            ...
            'quality_issues'=>array(),'rows'=>array(),'all_rows'=>array(),'confidence'=>array('algorithm'=>array(),'text_ai'=>array(),'image_ai'=>array()),
        );
        foreach($ids as $post_id) {
            ...
            $item=array('id'=>$post_id,'title'=>get_the_title($post_id),'method'=>$row['confirmed_stage']??'algorithm','confidence'=>$final_conf,'quality'=>$quality['score'],'issues'=>$quality['issues'],'human'=>$human,'status'=>$status,'image_used'=>!empty($row['image_ai']['used']));
            $a['all_rows'][]=$item;
            if (!$human || $quality['score']<80 || $final_conf<.70 || in_array($status,array('review','unclassified','ai_error'),true)) $a['rows'][]=$item;
        }
    ...
    private static function recommendations($a) {
        ...
        foreach(array_slice($a['quality_issues'],0,4,true) as $issue=>$count) {
            ...
            $out[]=array('title'=>$issue.'：'.$count.'件','body'=>$action,'impact'=>self::rate($count,$total).'%の商品が対象','url'=>add_query_arg(array('page'=>self::PAGE,'issue'=>$issue),admin_url('admin.php')).'#nf-ci-review');
        }
        if (($a['history']['route']['image_ai']??0)>0) $out[]=array('title'=>'Image AI監査を限定利用中','body'=>'画像AIは全商品ではなく、テキストだけで確定困難な商品に限定されています。二重チェックの不一致商品を優先確認してください。','impact'=>intval($a['history']['route']['image_ai']).'件で使用','url'=>add_query_arg(array('page'=>self::PAGE,'issue'=>'__image_used'),admin_url('admin.php')).'#nf-ci-review');
        return $out;
    }
    ...
    public static function page() {
        ...
        $review_rows=$a['rows'];
        if($issue_filter!=='') $review_rows=array_values(array_filter($a['all_rows'],function($item)use($issue_filter){ return $issue_filter==='__image_used'?!empty($item['image_used']):in_array($issue_filter,(array)$item['issues'],true); }));
        ...
        <section class="nf-ci-panel"><h2>改善提案</h2><div class="nf-ci-recommendations"><?php foreach($recs as $rec): ?><article><strong><?php echo esc_html($rec['title']); ?></strong><p><?php echo esc_html($rec['body']); ?></p><small><?php echo esc_html($rec['impact']); ?></small><p><a class="button" href="<?php echo esc_url($rec['url']); ?>">対象商品を見る</a></p></article><?php endforeach; ?><?php if(!$recs): ?><p>現在、優先度の高い改善提案はありません。</p><?php endif; ?></div></section>
        <section class="nf-ci-panel" id="nf-ci-review"><h2><?php echo $issue_filter!==''?'改善対象商品':'要確認商品'; ?></h2><?php if($issue_filter!==''): ?><p><strong>絞り込み：</strong><?php echo esc_html($issue_filter==='__image_used'?'Image AI使用商品':$issue_filter); ?>（<?php echo count($review_rows); ?>件） <a href="<?php echo esc_url(add_query_arg('page',self::PAGE,admin_url('admin.php')).'#nf-ci-review'); ?>">絞り込みを解除</a></p><?php endif; ?><table class="widefat striped"><thead><tr><th>商品</th><th>判定方式</th><th>Final confidence</th><th>品質</th><th>問題</th><th>人間確認</th></tr></thead><tbody><?php foreach(array_slice($review_rows,0,500) as $item): $primary_url=current_user_can('edit_post',$item['id'])?get_edit_post_link($item['id']):get_permalink($item['id']); ?><tr><td><strong><a href="<?php echo esc_url($primary_url); ?>"><?php echo esc_html($item['title']); ?></a></strong><small>ID: <?php echo intval($item['id']); ?> ／ <a href="<?php echo esc_url(get_permalink($item['id'])); ?>" target="_blank" rel="noopener">公開ページ</a></small></td><td><?php echo esc_html($item['method']); ?></td><td><?php echo esc_html(number_format_i18n(100*$item['confidence'],1)); ?>%</td><td><?php echo intval($item['quality']); ?>/100</td><td><?php echo esc_html(implode('／',$item['issues'])); ?></td><td><?php if($item['human']): ?><p>確認済み</p><?php elseif(self::can_review()): ?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="nf_intelligence_confirm"><input type="hidden" name="verification" value="correct"><input type="hidden" name="post_id" value="<?php echo intval($item['id']); ?>"><?php wp_nonce_field('nf_intelligence_confirm_'.$item['id']); ?><button class="button button-primary" type="submit">現在の分類を正解として確認</button></form><?php else: ?>未確認<?php endif; ?><?php if(self::can_review()): ?><p><a class="button" href="<?php echo esc_url(add_query_arg(array('page'=>self::PAGE,'correct_id'=>$item['id']),admin_url('admin.php')).'#nf-ci-correction'); ?>">分類を修正</a></p><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></section>
